...
/**
 * midtrans enrolment plugin tests.
 *
...
class enrol_midtrans_testcase extends advanced_testcase {

    protected function enable_plugin() {
        $enabled = enrol_get_plugins(true);
        $enabled['midtrans'] = true;
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['midtrans']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('midtrans'));
        $plugin = enrol_get_plugin('midtrans');
        $this->assertInstanceOf('enrol_midtrans_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_midtrans', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $midtransplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_midtrans_plugin $midtransplugin  */
        $midtransplugin = enrol_get_plugin('midtrans');
        ...
        $midtransplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $midtransplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $midtransplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $midtransplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $midtransplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $midtransplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $midtransplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $midtransplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $midtransplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $midtransplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $midtransplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $midtransplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $midtransplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the midtrans enrol plugin.
        ...
        // Midtrans enrolment has 2 enrol actions for active users when logged in as admin: edit and unenrol.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // Teachers don't have the enrol/midtrans:unenrol capability by default, but have enrol/midtrans:manage.
