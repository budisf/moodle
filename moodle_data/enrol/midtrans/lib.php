...
class enrol_midtrans_plugin extends enrol_plugin {
    ...
    public function get_currencies() {
        // See https://www.midtrans.com/cgi-bin/webscr?cmd=p/sell/mc/mc_intro-outside,
        // 3-character ISO-4217: https://cms.midtrans.com/us/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_api_currency_codes
        $codes = array(
            'IDR','AUD', 'BRL', 'CAD', 'CHF', 'CZK', 'DKK', 'EUR', 'GBP', 'HKD', 'HUF', 'ILS', 'INR', 'JPY',
    ...
    public function allow_unenrol(stdClass $instance) {
        // users with unenrol cap may unenrol other users manually - requires enrol/midtrans:unenrol
        return true;
    }

    public function allow_manage(stdClass $instance) {
        // users with manage cap may tweak period and status - requires enrol/midtrans:manage
        return true;
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/midtrans:config', $context)) {
            return false;
        }
    ...
    function enrol_page_hook(stdClass $instance) {
        ...
        if (abs($cost) < 0.01) { // no cost, other enrolment methods (instances) should be used
            ...
            // Calculate localised and "." cost, make sure we send Midtrans the same value,
            // please note Midtrans expects amount with 2 decimal places and "." separator.
            ...
            if (isguestuser()) { // force login only for guest user, not real users with guest role
                ...
                //Sanitise some fields before building the Midtrans form
                $courseid        = $course->id;
                ...
                //Create Random Order ID here
                $data = json_encode([
                    'transaction_details' => [
                        'order_id' => 'TRX-' . strtoupper(uniqid()) . $userid,
                        'gross_amount' => $localisedcost
                    ],
                    "item_details" => [
                        [
                            "id" => $instance->id,
                    ...
                    'custom_field1' => $roleid,
                    'custom_field2' => $userid,
                    'custom_field3' => $courseid
                ]);

                // var_dump($roleid);

                //Setup cURL for calling API and get Token 
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    ...
                    //Place Auth Key from Midtrans here
                ...
                $transactiontoken = '';
                //Place Token to variable to be called in File 
                if(isset($result_arr['token']) && $result_arr['token']) {
                    $transactiontoken = $result_arr['token'];
                }

                //Call the File with returned Token here
                include($CFG->dirroot.'/enrol/midtrans/enrol.html');
            }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $midtranscurrencies = $this->get_currencies();
        $mform->addElement('select', 'currency', get_string('currency', 'enrol_midtrans'), $midtranscurrencies);
