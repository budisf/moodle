...
class enrol_paypal_plugin extends enrol_plugin {
    ...
    public function get_currencies() {
        ...
        $codes = array(
            ...
            'MXN', 'MYR', 'NOK', 'NZD', 'PHP', 'PLN', 'RUB', 'SEK', 'SGD', 'THB', 'TRY', 'TWD', 'USD', 'IDR');
        $currencies = array();
        foreach ($codes as $c) {
        ...
        if (abs($cost) < 0.01) { // no cost, other enrolment methods (instances) should be used
            ...
            if (isguestuser()) { // force login only for guest user, not real users with guest role
                ...
                $data = json_encode([
                    ...
                    "custom_field1" => $instance->courseid,
                    "custom_field2" => $instance->roleid,
                    "custom_field3" => $USER->id
                ]);

                $url = "https://app.sandbox.midtrans.com/snap/v1/transactions";
                $serverKey = 'SB-Mid-server-TTEgc2WFeTGA-53mKvifCHgw';
                ...
                // $data_signature = "'".$order_id.$status_code.$gross_amount.$serverKey."'";
                // $signature_key = hash("sha512", $data_signature);

                // $record = new stdClass();
                // $record->course_id = $instance->courseid;
                // $record->user_id = $USER->id;
                // $record->signature_key = $signature_key;

                // $lastinsertid = $DB->insert_record('enrol_course', $record);
    ...
    public function cek_status($order_id){
        ...
        // $order_id = $data_dec->transaction_details->order_id;
        ...
        return $status = $result_arr['transaction_status'];
